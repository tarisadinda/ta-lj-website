...
export default function DetailVerification() {
    ...
    const getDetail = () => {
        axiosInstance.get(API_UNVERIF_COMPANY + "/" + pathUser)
        .then((res) => {
            console.log(res)
    ...
    return(<>
        ...
        <Card className={styles.infoWrapper} variant="outlined">
            <Image src={`http://localhost/images/${data?.img}`} className={styles.logoCompany} width={150} height={150} alt="company-logo" />
        ...
        <div className="mt-3">
            ...
            <div>
                ...
                <div className="row">
                    ...
                    <p className="col-6">{data?.company_detail.address}</p>
                </div>
            </div>
        </div>
    </>)
